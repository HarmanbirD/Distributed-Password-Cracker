...
int parse_arguments(int argc, char *argv[], arguments *args, struct fsm_error *err)
{
    ...
    // while ((opt = getopt(argc, argv, ":H:c:p:s:w:t:h")) != -1)
    {
        switch (opt)
        {
            case 'H':
...
int handle_arguments(const char *binary_name, arguments *args, struct fsm_error *err)
{
    ...
    if (args->work_size_str == NULL)
        ...
        string_to_int(args->work_size_str, &args->work_size);

    if (args->checkpoint_str == NULL)
        args->checkpoint = args->work_size / 4;
    else
        string_to_int(args->checkpoint_str, &args->checkpoint);

    if (args->timeout_str == NULL)
        args->timeout = 600;
    else
        string_to_int(args->timeout_str, &args->timeout);
